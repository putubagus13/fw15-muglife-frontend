  ...
  return (
    <>
      ...
      <main className='pt-28'>
        <div className={`${styles.bg_payment} w-full bg-no-repeat bg-cover py-11 px-7 lg:px-36 xl:px-64`}>
                ...
                {transaction[0]?.items?.map((item) => {
                  return (
                    <div key={`item-product-${item?.code}`} className='w-full h-24 flex items-start justify-between gap-7 '>
                      {item?.picture ? <Image width={96} height={96} src={item?.picture} className='rounded-full h-[96px] w-[96px]' alt='Product-Image' /> : <Image src={ProductImage} className='rounded-full h-[96px] w-[96px]' alt='Product-Image' />}
                      <div className='flex flex-col items-start justify-between max-w-[188px] overflow-hidden'>
                        ...
                        <div className='text-primary text-lg'>{item?.code === 'R' ? 'Regular' : item?.code === 'L' ? 'Large' : item?.code === 'XL' ? 'Extra Large' : item?.variant}</div>
                      </div>
